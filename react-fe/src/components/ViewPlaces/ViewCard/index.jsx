import React from "react";
//import { useNavigate } from "react-router-dom";
import axios from "axios";
import { ToastContainer, toast } from "react-toastify";
import "react-toastify/dist/ReactToastify.css";

const ViewCard = ({ placeItem }) => {
  const deletePlace = async (placeItem) => {
    const deleteParams = {
      placeId: placeItem._id,
      userId: sessionStorage.getItem("userId"),
    };

    try {
      const res = await axios.delete(
        `${process.env.REACT_APP_OUR_PLACES_URL}/places`,
        { params: deleteParams }
      );
      const data = res.data;
      toast.success(data.message);
    } catch (error) {
      toast.error(error?.response?.data?.error);
    }
  };

  const editPlace = async (placeItem) => {
    const editRequestData = {
      ...placeItem,
      userId: sessionStorage.getItem("userId"),
    };

    try {
      const res = await axios.patch(
        `${process.env.REACT_APP_OUR_PLACES_URL}/places/editPlace`,
        editRequestData
      );
      const data = res.data;
      toast.success(data.message);
    } catch (error) {
      toast.error(error?.response?.data?.error);
    }
  };
  return (
    <>
      <div
        className="mx-4"
        //onClick={() => navigate("/view-places")}
        key={placeItem.id}
      >
        <div className="card w-96 bg-base-100 shadow-xl rounded-md">
          <figure>
            <img
              src={
                placeItem.image ||
                "https://daisyui.com/images/stock/photo-1606107557195-0e29a4b5b4aa.jpg"
              }
              alt="Shoes"
              className="rounded-t-lg"
            />
          </figure>
          <div className="card-body text-left p-2 m-2">
            <div className="flex bg-accentinfo justify-between place-items-center">
              <h2 className="card-title text-2xl font-bold pb-1">
                {placeItem.title}
              </h2>
              <div className="flex flex-wrap">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  fill="none"
                  viewBox="0 0 24 24"
                  stroke-width="1.5"
                  stroke="currentColor"
                  className="w-6 h-6 text-green-900 hover:cursor-pointer"
                  onClick={() => editPlace(placeItem)}
                >
                  <path
                    stroke-linecap="round"
                    stroke-linejoin="round"
                    d="m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L10.582 16.07a4.5 4.5 0 0 1-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 0 1 1.13-1.897l8.932-8.931Zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0 1 15.75 21H5.25A2.25 2.25 0 0 1 3 18.75V8.25A2.25 2.25 0 0 1 5.25 6H10"
                  />
                </svg>
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  fill="none"
                  viewBox="0 0 24 24"
                  stroke-width="1.5"
                  stroke="currentColor"
                  className="w-6 h-6 text-red-900 hover:cursor-pointer"
                  onClick={() => deletePlace(placeItem)}
                >
                  <path
                    stroke-linecap="round"
                    stroke-linejoin="round"
                    d="m14.74 9-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 0 1-2.244 2.077H8.084a2.25 2.25 0 0 1-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 0 0-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 0 1 3.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 0 0-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 0 0-7.5 0"
                  />
                </svg>
              </div>
            </div>
            <p>{placeItem.description.length > 140 ? placeItem.description.slice(0, 140)+'...' : placeItem.description}</p>
            <div className="card-actions justify-end">
              <button className="btn btn-accent">View Location</button>
            </div>
          </div>
        </div>
      </div>
    </>
  );
};

export default ViewCard;
